...
export function createWindowApi(overrides: Partial<Window['api']> = {}): Window['api'] {
  return {
    ...
    getAgentTraceCollectionStatus: vi.fn().mockResolvedValue({
      enabled: false,
      listening: false,
      endpoint: null,
      errorMessage: null,
    ...
    getAgentTraceSession: vi.fn().mockResolvedValue({
      source: 'vscode',
      sessionId: '',
      availability: 'not-collected',
      spans: [],
    }),
    clearAgentTraceData: vi.fn().mockResolvedValue(undefined),
